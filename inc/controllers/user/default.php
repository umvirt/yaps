...
class defaultController extends \Yaps\YapsController
{
     ...
     */
    public function setlocaleCmdAct()
    {
        global $locales;
    ...
    */
    ...
    public function defaultAct()
    {
        ...
        if ($islogined) {
            // print menu for user
            echo rstr(_('YAPS_CURRENTUSER_MESSAGE'), ['login' => $_SESSION['login'], 'link2exit' => $this->yaps->localLink("/user/logout")]);

            // if not logined
        } else {
            //print menu for visitor
            echo rstr(_('YAPS_VISITOR_MESSAGE'), ['link2login' => $this->yaps->localLink("/user/loginfrm")]);
        }
